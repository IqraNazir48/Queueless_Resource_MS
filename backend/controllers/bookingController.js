// backend/controllers/bookingController.js - FIXED TIME VALIDATION
...
// UPDATED: Check if a slot's START time has passed
...
exports.bookSlot = async (req, res) => {
    try {
        ...
        // UPDATED: Check if slot start time has passed
        if (isPastBooking(date, slot)) {
            return res.status(400).json({ 
                message: "Cannot book slots that have already started. Please select a current or future time slot." 
            });
        }
